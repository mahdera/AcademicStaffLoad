...
//here do the calculation Mahder
...
//create a header for the excel file
	...
	$objPHPExcel->getActiveSheet()->setCellValue('C1','Expected Semester Load');
	$objPHPExcel->getActiveSheet()->setCellValue('D1','Undergrad Course Load');
	$objPHPExcel->getActiveSheet()->setCellValue('E1','Postgrad Course Load');
	$objPHPExcel->getActiveSheet()->setCellValue('F1','Undergrad Advising Load');
	$objPHPExcel->getActiveSheet()->setCellValue('G1','Postgrad Project Advising Load');
	$objPHPExcel->getActiveSheet()->setCellValue('H1','Thesis Advising Load');
	$objPHPExcel->getActiveSheet()->setCellValue('I1','Total Advising Load');
	$objPHPExcel->getActiveSheet()->setCellValue('J1','Total Semester Load');
	$objPHPExcel->getActiveSheet()->setCellValue('K1','Semester Excess Load');
	$objPHPExcel->getActiveSheet()->setCellValue('L1','Advising Excess Load');
	$objPHPExcel->getActiveSheet()->setCellValue('M1','Course Excess Load');
	...
	while($resultRow = mysql_fetch_object($result))//iterate thru all the instrcutors to check that if they have a load or not!
	{
			...
			if($ans == "Yes")
			{
				...
				$objPHPExcel->getActiveSheet()->setCellValue($colC,$expectedSemesterLoad);
				$objPHPExcel->getActiveSheet()->setCellValue($colD,$undergradCourseLoad);
				$objPHPExcel->getActiveSheet()->setCellValue($colE,$postgradCourseLoad);
				$objPHPExcel->getActiveSheet()->setCellValue($colF,$undergradAdvisingLoad);
				$objPHPExcel->getActiveSheet()->setCellValue($colG,$postgradProjectAdvising);
				$objPHPExcel->getActiveSheet()->setCellValue($colH,$thesisAdvisingLoad);
				$objPHPExcel->getActiveSheet()->setCellValue($colI,$totalAdvisingLoad);
				$objPHPExcel->getActiveSheet()->setCellValue($colJ,$totalSemesterLoad);
				if($semesterExcessLoad < 0.0)
					$semesterExcessLoad = "---";
				$objPHPExcel->getActiveSheet()->setCellValue($colK,$semesterExcessLoad);
				if($AEL < 0.0)
					$AEL = "---";
				$objPHPExcel->getActiveSheet()->setCellValue($colL,$AEL);
				//i can check if the semesterExcessLoad is < 0 or not
				if($CEL < 0.0)
					$CEL = "---";
				$objPHPExcel->getActiveSheet()->setCellValue($colM,$CEL);
...
//print("<a href='ExportCourseReportToExcel.xlsx'>View Excel Document here</a>");
  Header("Location: ExportCurrentSemesterLoadReportToExcelForThisAcademicUnit.xlsx");
