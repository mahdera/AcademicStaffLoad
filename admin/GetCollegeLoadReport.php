				...
				print("<table width='100%' border='1'>");
					...
					while($resultRow = mysql_fetch_object($resultInstructors))//iterate thru all the instrcutors to check that if they have a load or not!
					{
						...
						if($ans == "Yes")
						{
							...
							$resultSemesterLoad = DBConnection::readFromDatabase($query);
							$resultSemesterLoadRow = mysql_fetch_object($resultSemesterLoad);
							//var_dump($resultSemesterLoadRow);

							$fullName = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->full_name : "";
							$normalCourseLoad = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->normal_course_load : 0.0;
							$expectedSemesterLoad = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->expected_semester_load : 0.0;
							$postgradCourseLoad = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->post_grad_course_load : 0.0;
							$additionalResponsibilityWaiver = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->additional_responsibility_weaver : 0.0;
							$undergradCourseLoad = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->undergrad_course_load : 0.0;
							$undergradAdvisingLoad = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->undergrad_advising_load : 0.0;
							$postgradProjectAdvising = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->post_grad_project_advising : 0.0;
							$thesisAdvisingLoad = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->thesis_advising_load : 0.0;
							$totalAdvisingLoad = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->total_advising_load : 0.0;
							$totalSemesterLoad = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->total_semester_load : 0.0;
							$semesterExcessLoad = $resultSemesterLoadRow != null ? $resultSemesterLoadRow->semester_excess_load : 0.0;
							//i can compute the additional computation wondisho brought in here
							...
							if($ctr % 2 == 0)
						   {
							...
							while($resultLoadRow = mysql_fetch_object($resultLoad)){
								if($resultLoadRow != null){
									$courseNumber = $resultLoadRow->course_number;
									$courseDetailRow = CourseOffering::getCourseDetail($courseNumber);
									if($courseDetailRow != null){
										$creditHour = $courseDetailRow->credit_hour;
										$lectureHour = $courseDetailRow->lecture_hour;
										$labHour = $courseDetailRow->lab_hour;
										$tutorialHour = $courseDetailRow->tutorial_hour;
